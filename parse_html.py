...
def extract_content_from_page(file_path):
    ...
    #Extracting Dates
    parsed_data['date']= (soup.find("span", {"class": "tie-date"}).text)
   
    #Extracting Title 

    parsed_data['title'] = (soup.find("h1", {"class": "name post-title entry-title"}).text)

    #Extracting Content

    parsed_data['content'] = (soup.find("div", {"class": "entry"}).text)
