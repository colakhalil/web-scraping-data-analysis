...
def download_links_from_index():
    ...
    if not os.path.exists(LINK_LIST_PATH):
        ...
        start_page = 0 #I have changed this part as zero because I will start my loop from start_page + 1 to fix connection problem issues
        downloaded_url_list = []
    ...
    rootURL = 'https://www.mfa.gov.bt/press-releases/?_page=' #Add ID


    for pid in range(start_page + 1,11):
        ...
        for item in soup.find_all("h5", {"class": "pt-cv-title"}):

            # Save the collected url in the variable "collected_url"
            collected_url = item.find("a")["href"]
